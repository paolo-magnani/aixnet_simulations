...
def file_loader(estimator): # load files choosing the right estimator; further changes are needed to use one specific data set (e.g., using EPOS-LHC)
    ...
    if estimator=='AixNet':
    ...
    elif estimator=='AixNet_EPOS_test':
    ...
    elif estimator=='AixNet_EPOS_old':
        input_path = '/mnt/c/Users/paolo/Desktop/LAVORO/Berenika/AixNet_EPOSold_refStSl_sims.csv'
        kf = pd.read_csv(input_path)
        dd = {}
        dd['dnn_xmax'] = np.array(kf['Xmax'])
        dd['mc_xmax'] = np.array(kf['Xmax_MC_int'])
        ...
        mask = (dd['mass']!=16)|((dd['dnn_xmax']<1000)&(dd['mass']==16))
        dd = dict_cutter(dd, mask)
...
class dipole_parameters:
    ...
    vec_b_r = np.arange(0.1, 3.1, step=0.01)
...
class dipole_parameters_emily:
    # this class is given by f(Z,E)*J(E)*d(Z,E), and together they should minimize distance with the dipole values
    # using Emily's recipe
            ...
            for beta_r in self.vec_b_r:
                ...
                for i in range(len(enebins[:-1])):
                    ...
                    for name in charges:
                        ...
                        if name == 'Fe':
                            Z_sum = self.dipole_func(name, E, d_r, beta_r, self.dmax)
                            mean_frac=np.mean(1 - (self.mf.fraction_func('p', np.log10(E)+18) + self.mf.fraction_func('He', np.log10(E)+18) + self.mf.fraction_func('CNO', np.log10(E)+18)))
                            dip += mean_frac*np.sum(Z_sum*self.es.spectrum_func_simpl(E))/np.sum(self.es.spectrum_func_simpl(E))
                            test += mean_frac
                        else:
                            Z_sum = self.dipole_func(name, E, d_r, beta_r, self.dmax)
                            mean_frac= np.mean(self.mf.fraction_func(name, np.log10(E)+18))
                            dip += mean_frac*np.sum(Z_sum*self.es.spectrum_func_simpl(E))/np.sum(self.es.spectrum_func_simpl(E))
                            test += mean_frac
